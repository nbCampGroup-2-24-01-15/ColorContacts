...
class Notification: BroadcastReceiver() {
    ...
    fun setUserAlarm(user: User,context: Context) {
        ...
        alarmIntent.putExtra("KEY", user.key)
        ...
        val pendingIntent = PendingIntent.getBroadcast(
            context,
            0, //pendingIntent 를 식별하는 코드
            alarmIntent,// 알람이 트리거 될때 실행되는 intent
            PendingIntent.FLAG_IMMUTABLE // 생성된 PendingIntent 는 불가변성이다.
        )

        alarmManager.setExactAndAllowWhileIdle( // 정확한 시간에 아람이 발생하는 메소드
